...
def show():
    ...
    if st.button("🚀 تحليل والعبور"):
        ...
        # حساب النتيجة
        sfm_val = (eco + soc + env) / 3
        
        # فحص الشروط
        is_passed, reason, violations = check_gate_2_conditions(
            risk_score, sustainability, npv, sfm_val
        )
        ...
        if is_passed:
            st.balloons()
            st.success(f"✅ اجتاز المشروع البوابة بنتيجة {sfm_val:.1f}%")
        else:
        ...
        payload = {
            ...
            "score": round(sfm_val, 2),
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        
        res = send_to_n8n_webhook(payload)
        
        with st.expander("🔗 حالة الربط"):
            if res.get('success'):
                st.write("✅ تم التحديث في المحرك الذكي")
            else:
                st.write("⚠️ المعالجة تمت محلياً فقط")
